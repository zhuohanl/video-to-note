...
export function ReviewClient({ jobId }: { jobId: string }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const saved = await putNote(jobId, note.etag, markdown);
      setNote(saved);
    }, AUTOSAVE_MS);
  ...
  async function patchContent(body: { include_summary?: boolean; include_transcript?: boolean }) {
    ...
    try {
      const updated = await patchNote(jobId, note.etag, body);
      setNote(updated);
  ...
  async function rebuild() {
    ...
    const updated = await rebuildNote(jobId, note.etag);
    setNote(updated);
    setMarkdown(updated.markdown);
  }
  ...
  async function keep() {
    ...
    const updated = await keepNote(jobId, note.etag);
    setNote(updated);
  }
  ...
  async function saveCurrentVersion() {
    ...
    const saved = await saveVersion(jobId, note.etag, clipsView.collection_etag, null);
  ...
  async function restore(seq: number) {
    ...
    await restoreVersion(jobId, seq, note.etag, clipsView.collection_etag);
  }
